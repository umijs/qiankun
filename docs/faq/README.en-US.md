...
1. check you have exported the specified lifecycles, see the [doc](/en-US/guide/getting-started#1-exports-lifecycles-from-sub-app-entry)

2. check you have set the specified configuration with your bundler, see the [doc](/en-US/guide/getting-started#2-config-sub-app-bundler)
...
8. Check whether the main app and micro-app use AMD or CommonJS. Check method: run the main app and the micro-app independently, and enter the following code in the console: `(typeof exports === 'object' && typeof module === 'object') || (typeof define === 'function' && define.amd) || typeof exports === 'object'`，If it returns `true`，that it is caused by this reason, and there are mainly the following two solutions:
    ...
    ```
    - Solution 2: The micro-app is not bundle with `umd`, directly mount the life cycle function to the `window` in the entry file, refer to[Micro app built without webpack](/en-US/guide/tutorial#micro-app-built-without-webpack).
...
You can find the singular usage [here](/en-US/api#startopts).
...
- If you load the microapps through [registerMicroApps](/en-US/api#registermicroappsapps-lifecycles), you need to configure a custom fetch in the start method, such as:
...
- If you load the microapp via [loadMicroApp](/en-US/api#loadmicroappapp-configuration), you need to configure a custom fetch when invoking, such as:
