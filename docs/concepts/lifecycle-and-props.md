...
**Remount (mount after unmount).** The app's DOM is cleared on unmount and rebuilt on the next mount, and the sandbox is deactivated then reactivated. But the two execution paths handle the _code_ differently:

- **Classic** — the entry script is **re-executed** on each remount. Top-level code runs again every time.
- **ESM** — the module graph is **not** re-run. Remount reuses the same blob URL, and `import(sameBlobUrl)` returns the same module namespace, so top-level module code executes only once for the app's lifetime. Only `mount(props)` runs again.

::: warning ESM apps must create state inside mount()
Because ESM top-level code runs once, an ESM sub-app that instantiates its framework app or global state at module top level will not re-create it on remount. Create the app instance _inside_ `mount()` and dispose it in `unmount()`. This is the modern-framework pattern anyway, and it matches how [the ESM sandbox](/concepts/esm-sandbox) caches modules.
:::
...
::: info loadMicroApp has no unload
Parcels created by `loadMicroApp` have no `unload` semantics, so their ESM engine lingers until you drop the reference to the returned handle. Always call `unmount()` on handles you no longer need. See [Run multiple micro-app instances](/cookbook/run-multiple-instances).
:::
