...
- 类型
  ...
  - `RegistrableApp`
    ...
    - activeRule - `string | (location: Location) => boolean | Array<string | (location: Location) => boolean> ` - 必选，微应用的激活规则。
      ...
      * 支持直接配置字符串或字符串数组，如 `activeRule: '/app1'` 或 `activeRule: ['/app1', '/app2']`，当配置为字符串时会直接跟 url 中的路径部分做**前缀匹配**，匹配成功表明当前应用会被激活。
      ...
      * 注意，此处的前缀匹配指的是字符串形式的前缀匹配，而非对子路由进行前缀匹配。如，对`/app1`来说，匹配规则为`/app1*`，而非`/app1/*`（`*`为通配符）。
