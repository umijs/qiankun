...
- There is no `FrameworkConfiguration` type. Per-app configuration is `AppConfiguration`; `start()` takes only single-spa's `StartOpts`.
...
| `AppConfiguration` | loader options `& { sandbox?; loadTimeout? }` | Per-app runtime configuration. See [AppConfiguration](/api/configuration). |
