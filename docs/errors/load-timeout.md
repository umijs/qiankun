...
The micro app has a `timeout`, either configured directly or inherited from `start({ timeout })`, and loading did not finish within that limit.
...
The error is a `LoadAppTimeoutError`. It extends `QiankunError` and adds the read-only fields `appName`, `timeout`, and `elapsed`.

## Troubleshooting

1. Compare `timeout` with `elapsed` to check whether the limit is much shorter than a normal load.
...
Adjust the limit to match your network conditions and asset sizes, or set `timeout` for individual apps only. Set it to `0` to disable the timeout for an app.
