...
代码继续操作已经 `dispose()` 的 Compartment 或 ESM 执行实例，或者销毁取消了尚未完成的任务。常见情况包括：

- `dispose()` 取消了尚未完成的经典脚本求值；
- `dispose()` 时 ESM 入口模块仍在加载，入口 Promise 随之拒绝；
- 沙箱销毁后，仍有代码通过它保存的资源转换函数或 `fetch` 插入脚本、样式或发起请求，例如微应用卸载后才触发的回调；
- 沙箱控制器 `dispose()` 之后又调用了 `mount()` 或 `nodeTransformer`，或者 `dispose()` 发生时 `mount()` 仍在执行。

## 排查步骤

1. 根据调用栈确定失败的是脚本求值、模块导入、动态插入的资源，还是其他实例操作。
