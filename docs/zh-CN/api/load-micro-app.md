...
- **刚失败的应用不会立即重新加载。** 同一 `name` 和 `entry` 的加载或 `bootstrap` 失败后，约 1 秒内再次调用 `loadMicroApp` 不会被拒绝，但加载会延后到这段冷却期结束再开始，避免重渲染循环或密集重试反复请求入口。冷却期从失败时开始计算，不因新的调用而延长；等待期间调用 `unload()` 会取消这次加载。
