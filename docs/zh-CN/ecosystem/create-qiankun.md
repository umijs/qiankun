...
`create-qiankun` 是 qiankun v3 的官方脚手架，用来创建可以直接配合运行的 Vite 主应用和微应用。
...
微应用使用 `@qiankunjs/bundler-plugin/vite` 准备开发服务器和 HTML Entry。普通的 `dev`、`build`、`preview` 产物都可以被 qiankun 加载，不需要单独的 SystemJS 或 UMD 构建模式。

生成文件的代码是可修改的起点，不是新的公共 API。理解接入契约时，应以[生命周期与 props](/zh-CN/concepts/lifecycle-and-props)和 [`loadMicroApp` API](/zh-CN/api/load-micro-app)为准。

## 默认连接方式

主应用默认从 `//localhost:7101` 加载微应用，并把返回句柄绑定到 React 组件的生命周期：容器创建后调用 `loadMicroApp`，组件清理时调用 `unmount()`。

如果需要多个微应用，请为每个开发服务器设置不同端口，并为每个实例提供独立容器。参见[运行多个微应用实例](/zh-CN/cookbook/run-multiple-instances)。

路由完全决定应用是否激活时，可以把主应用改为 [`registerMicroApps`](/zh-CN/api/register-micro-apps) 和 [`start`](/zh-CN/api/start)；它们不是默认生成路径的前置步骤。
...
打开 `http://localhost:7099` 查看组合后的页面，或打开 `http://localhost:7101` 单独开发微应用。完整过程见[快速上手](/zh-CN/guide/getting-started)。
