...
qiankun 还会在当前页面内维护一个内存 LRU 缓存，保存成功的请求结果，并按应用名称复用已卸载的实例。只有运行时缓存未命中时，HTTP 协商缓存才会参与请求；同一页面中的重新挂载不能用于刷新部署版本。如需确保加载新发布的微应用，应刷新主应用页面。
...
同一应用再次挂载时，qiankun 按 `name` 复用已卸载的实例：即使容器是新创建的节点，也不会重新加载和执行入口。因此同一 `name` 应始终指向同一个应用，不要在每次渲染时为同一应用生成不同的名称。
