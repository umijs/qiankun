...
> 本页面向维护者，记录生命周期发现与编排的实现细节。面向使用者的约定见[微应用生命周期与 props](/zh-CN/concepts/lifecycle-and-props)。
...
**重挂(unmount 之后再 mount)。** 应用的 DOM 在 unmount 时被清空、在下一次 mount 时重建，沙箱也随之停用、再激活。重新加载的 HTML 会移除脚本节点，因此两条执行路径都会复用首次加载时发现的生命周期对象：Classic 入口代码和 ESM 模块顶层代码都不会再次执行，只有 `mount(props)` 会再跑。

::: warning 在 mount() 中创建可销毁的状态
因为顶层代码不会再次执行，在模块顶层实例化框架应用或视图状态的子应用，重挂时不会重新创建它们。请把应用实例的创建放*进* `mount()`，并在 `unmount()` 中销毁。
:::
...
::: info loadMicroApp 不暴露 unload
`loadMicroApp` 返回的公开句柄不暴露 single-spa 的 `unload` 生命周期。用不到的句柄应始终调用 `unmount()`，但不要把它视为 ESM 引擎的完整销毁。见[同时运行多个微应用实例](/zh-CN/cookbook/run-multiple-instances)。
:::
