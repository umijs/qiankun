...
暂时移除微应用时调用 `unmount()`，保留缓存和沙箱状态以便重新挂载。不再需要该代实例时调用 `unload()`，销毁同名应用在同一容器中的整代实例；之后需要重新调用 `loadMicroApp`。直接丢弃句柄不能替代清理。详见 [loadMicroApp](/zh-CN/api/load-micro-app#unload)。
