...
| 重新挂载 | 整段脚本重跑 | 顶层只跑一次；`import(sameBlob)` 返回同一个 namespace |
...
- **重新挂载不会重跑顶层代码。** `import(sameBlobUrl)` 返回的是*同一个*模块 namespace，所以一个模块的顶层只执行一次——只有 `mount(props)` 会再跑。任何每实例的状态（应用实例、store、router）都必须在 `mount()` 里创建，不能放在模块作用域。经典应用每次重新挂载都重跑整段脚本，没有这条约束。
...
- **Firefox 需要开个开关。** 多份动态注入的 import map 需要 `dom.multiple_import_maps.enabled`，而它在 Firefox 里默认是关的。Chrome/Edge 133+ 和 Safari 18.4+ 原生支持。想在 Firefox 和更老的浏览器上都拿到确定的支持，就把 [es-module-shims](https://github.com/guybedford/es-module-shims) 作为一个受支持的基座引入，而不是当成一个可选的兜底。
