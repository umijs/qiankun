...
const getAssetCache = once(() => new LRUCache<string, CacheEntry>(50));
...
const getModuleCache = once(() => new LRUCache<string, CacheEntry>(512));
...
export const makeFetchCacheable = (
  ...
  const lruCache = options.scope === 'modules' ? getModuleCache() : getAssetCache();
