...
const getAssetCache = once((): CachePool => new LRUCache<string, CacheEntry>(50));
...
const getModuleCache = once((): CachePool => new LRUCache<string, CacheEntry>(512));

/** A signal belongs to this wrapper's lifetime, independently of other cache users. */
export const makeFetchCacheable = (
  fetch: Fetch,
  options: { scope?: 'assets' | 'modules'; signal?: AbortSignal } = {},
): CacheableFetch => {
  const pool = options.scope === 'modules' ? getModuleCache() : getAssetCache();
