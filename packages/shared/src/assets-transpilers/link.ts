...
export default function transpileLink(
  ...
  /*
   * While the ESM sandbox takes over module loading, modulepreload links (from the sub app HTML or
   * generated by the streaming loader) would fetch the original URLs through the browser internal
   * loader, which can never be reused by the rewrite pipeline (RFC §10.1) — suppress them and let the
   * engine fetch through the cacheable fetch instead.
   */
  if (opts.esmEngine && link.rel === 'modulepreload') {
    link.removeAttribute('href');
    link.dataset.href = hrefAttribute ?? '';
    link.dataset.consumed = 'true';
    return link;
  }
