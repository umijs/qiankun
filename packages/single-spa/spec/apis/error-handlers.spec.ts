...
describe('error handlers api', () => {
  ...
  it(`reports an error during unmount`, () => {
    ...
    return singleSpa
      ...
      .then(() => {
        ...
        expect(errs[0].message.indexOf(`'unmount-error' died in status UNMOUNTING: couldn't unmount`)).toBeGreaterThan(
          -1,
        );
      });
  });
  ...
  it(`reports an error during activity functions`, () => {
    ...
    return singleSpa.triggerAppChange().then(() => {
      ...
      expect(
        errs[0].message.indexOf(`'activity-error' died in status NOT_LOADED: bad activity function`),
      ).toBeGreaterThan(-1);
    });
  });
