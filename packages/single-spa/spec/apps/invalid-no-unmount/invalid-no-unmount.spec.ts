...
describe(`invalid-no-unmount app`, () => {
  ...
  beforeEach(() => {
    ...
    return import('./invalid-no-unmount.app').then((app) => (myApp = app)).then((app) => app.reset());
  });
  ...
  it(`is never initialized`, () => {
    return singleSpa.triggerAppChange().then(() => {
      ...
      expect(singleSpa.getAppStatus('./invalid-no-unmount.app')).toEqual('SKIP_BECAUSE_BROKEN');
    });
  });
});
