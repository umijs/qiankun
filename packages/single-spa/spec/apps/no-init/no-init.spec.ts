...
describe(`no-init app`, () => {
  ...
  it(`allows an application to omit the init lifecycle`, async () => {
    expect(singleSpa.getAppStatus('no-init')).toBe(singleSpa.AppOrParcelStatus.NOT_LOADED);

    location.hash = activeHash;

    await singleSpa.triggerAppChange();

    expect(singleSpa.getAppStatus('no-init')).toBe(singleSpa.AppOrParcelStatus.MOUNTED);
  });
});
