...
describe(`delayed-start`, () => {
  ...
  it('will mount an application that began loading before start() was called', async () => {
    ...
    expect(singleSpa.getAppStatus('delayed-start')).toBe(singleSpa.AppOrParcelStatus.NOT_LOADED);

    await tick();

    expect(singleSpa.getAppStatus('delayed-start')).toBe(singleSpa.AppOrParcelStatus.LOADING_SOURCE_CODE);
    ...
    expect(singleSpa.getAppStatus('delayed-start')).toBe(singleSpa.AppOrParcelStatus.MOUNTED);
  });
});
