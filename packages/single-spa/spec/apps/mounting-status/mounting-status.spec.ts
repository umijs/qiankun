...
describe(`mounting-status`, () => {
  ...
  it(`puts application into MOUNTING status when it's mounting`, async () => {
    ...
    expect(singleSpa.getAppStatus('mounting-status')).toEqual(singleSpa.AppOrParcelStatus.NOT_LOADED);
    singleSpa.navigateToUrl(activeHash);
    const routingPromise = singleSpa.triggerAppChange();

    await mountStartPromise;
    expect(singleSpa.getAppStatus('mounting-status')).toEqual(singleSpa.AppOrParcelStatus.MOUNTING);

    await routingPromise;
    expect(singleSpa.getAppStatus('mounting-status')).toEqual(singleSpa.AppOrParcelStatus.MOUNTED);
