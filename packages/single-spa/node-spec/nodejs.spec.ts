...
describe(`nodejs spec`, () => {
  describe('activity functions', () => {
    ...
    it('can still check activity functions in nodejs with a mocked Location object', () => {
      ...
      expect(singleSpa.checkActivityFunctions({ pathname: '/app1' })).toEqual(['app1', 'navbar']);
      expect(singleSpa.checkActivityFunctions({ pathname: '/app2' })).toEqual(['app2', 'navbar']);
      expect(singleSpa.checkActivityFunctions({ pathname: '/app3' })).toEqual(['navbar']);
    });
